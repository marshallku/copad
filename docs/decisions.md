...
## 53. `force_meta_keys` config — explicit control-char whitelist for Opt-Meta, orthogonal to `option_as_alt`

**Problem.** macOS의 `option_as_alt = true`(default)는 Option+printable을 `ESC + base_char`로 보내지만, 의도적으로 control-character 키(Return, Escape, Tab, …)를 제외합니다 — `optionAltBytes`의 `v < 0x20` 필터. 이유: Cocoa의 `moveWordLeft:` / `deleteWordBackward:` 키 바인딩이 Opt+←/Opt+⌫을 readline 바이트로 변환하는 path를 보존해야 하기 때문(코드 주석 1893-1894).

부작용: Claude Code, Python REPL, ipython 등이 newline-in-prompt로 인식하는 `ESC + CR`(M-Enter) 시퀀스가 copad에서 전송되지 않습니다. 사용자가 Opt+Return을 치면 그냥 `\r`만 전송되어 prompt가 submit됩니다. 사용자 페인 포인트: "Opt+Return으로 줄바꿈이 안 됨."

**Considered.**

1. **Hardcode Opt+Return = ESC+CR.** Surgical 한 줄. 단점: 유사 케이스(Opt+Escape, 추후 추가될 키)가 나오면 매번 코드 수정.
2. **Control-char 필터 자체를 제거.** 모든 Opt+control 키가 ESC prefix를 받음. 단점: 위에서 말한 Cocoa 바인딩 경로를 깨뜨려 Opt+←/Opt+⌫이 무동작이 됨.
3. **`option_as_alt`를 polymorphic `bool | {enabled, force_meta_keys}` object로.** TOMLKit 디코딩 복잡도 + 기존 `option_as_alt = true|false`와의 마이그레이션 부담.
4. **별도 flat 필드 `force_meta_keys: [String]`.** Default `["Return"]`. ← 선택.

**Decision.** `[terminal] force_meta_keys = ["Return"]` (default) — control-character 키 중 명시된 것만 `optionAsAlt`의 printable-only 필터를 우회하여 `ESC + <byte>`를 PTY로 보냄. 인식 키(case-insensitive): `Return`/`Enter` → `[0x1B, 0x0D]`, `Escape`/`Esc` → `[0x1B, 0x1B]`. 알 수 없는 이름은 stderr 경고 후 무시.

`option_as_alt`와 **직교**(orthogonal): `option_as_alt = false` + `force_meta_keys = ["Return"]` 조합이 유효 — 악센트 입력을 위해 Option=diacritics를 유지하면서도 Opt+Return = newline은 살려둘 수 있음.

**Why not extend `option_as_alt`.**

- `option_as_alt = true|false`는 boolean으로 자리잡았고 기존 사용자 config에 그대로 적혀 있음. polymorphic 변환은 TOMLKit 디코딩이 복잡해지고 마이그레이션 부담.
- 두 개념(전역 Meta behavior vs. 특정 control 키 화이트리스트)은 의미적으로 직교적. flat 필드가 더 깨끗.

**Why default `["Return"]`, not `[]`.**

- 모든 macOS 사용자가 Claude Code/REPL 사용을 위해 동일한 한 줄을 추가해야 한다면 ergonomics 문제. "explicit configurability"는 *기능이 있는 것*이지 *default가 빈 것*은 아님.
- 끄려면 `force_meta_keys = []`로 명시 — 사용자가 명시적으로 끌 수 있는 길은 열려 있음.

**Why Return + Escape only.**

- Tab은 shell completion + readline + tmux prefix-Tab 등 conflict 표면이 넓음. Return/Escape는 `ESC + <byte>` 시퀀스가 거의 모든 환경에서 잘 정의되어 있음(Return=newline-in-prompt, Escape=ESC-ESC).
- Arrows/Delete는 절대 화이트리스트 금지 — Cocoa `interpretKeyEvents` path가 의존.
- Rule of Three: 추가 키 요청이 들어오면 그때 같은 패턴으로 확장(parser switch + KeyCode enum + `forceMetaBytes`).

**Trade-offs accepted.**

- **Config surface 추가 1개.** YAGNI를 살짝 어김(현재 알려진 케이스 1건). 하지만 사용자가 명시적 설정 가능성을 우선시함 — 키보드 동작은 사용자별 워크플로우 의존성이 크므로 노출 가치 있음.
- **Linux 미러링 안 함.** copad-linux는 VTE가 Alt 처리 자체를 갖고 있어 동일 문제 없음. macOS-only feature로 시작.
...
## 56. `[window] opacity` / `blur` — Ghostty-style window transparency (macOS)

**Decision.** Introduce a new `[window]` config section with `opacity: f64` (default `1.0`) and `blur: bool` (default `false`, macOS-only). On macOS, `opacity < 1.0` flips `NSWindow.isOpaque = false`; cell-painting in both renderers (Alacritty, SwiftTerm) skips default-bg cells so the alpha-tinted layer bg shows through; if `blur = true`, an `NSVisualEffectView` (material `.hudWindow`, blendingMode `.behindWindow`) installs as the contentView's bottom-most subview. Chrome (TabBar bar, StatusBar) picks up the same alpha; tab pills stay opaque.

**Why distinct from `[background] opacity`.** Existing `background.opacity` only affects the optional background-image layer (`background.rs:145` `if self.has_image.get()`). When there's no image, the knob is unused. Repurposing it for window-level transparency would silently change semantics for users who set it expecting "image opacity" — and would force "you must have an image set to get transparency" which Ghostty doesn't. A new key keeps both semantics intact and avoids a backwards-incompat re-mapping.

**Why a new cell-painting gate is not needed on Alacritty.** The renderer already had `transparentDefaultBg && imageBackgroundActive` plumbed through three sites (bounds fill, per-cell skip, cursor outline) for the wallpaper-passthrough path (Zed pattern). The change is mechanical: extract the OR (`isTransparentBgActive`) so window-opacity hits the same skip path, and route the layer-bg flip through a single `layerBg(theme:opacity:imageActive:)` helper so all three modes (opaque / image / window-alpha) share one place.

**Why `.hudWindow` for blur material.** Catppuccin Mocha + the other built-in themes are dark; `.hudWindow` reads as a dense dark vibrancy that doesn't fight the theme bg. `.fullScreenUI` (the system-default for a regular window) is too bright behind a dark terminal; `.menu` is too thin. Future light themes could route `appearance` luminance to pick `.menu` / `.popover`, but no light theme ships today so the simple fixed pick is fine.

**Why `blendingMode = .behindWindow`.** That's the mode that blends with whatever's *behind the window* (desktop, other apps) — the Ghostty behavior. `.withinWindow` would only blur sibling windows of the same app, which is not what we want.

**Linux deferred.** GTK4 supports translucent windows on compositors that allow it (`gtk_widget_set_opacity` + transparent style class), and VTE already paints with `set_clear_background(false)` + `RGBA(0,0,0,0)` for the image-bg path. But there's no NSVisualEffectView equivalent, and the compositor-dependent behavior wants its own decision pass (Wayland vs X11, KDE vs GNOME vs sway), so wire it in a follow-up PR once we know which compositors we care about. The config schema is cross-platform today so the Linux PR doesn't need to re-shape it.

**See:** `copad-core/src/config.rs::WindowConfig`, `copad-macos/Sources/Copad/AppDelegate.swift::applyWindowTransparency` + `installOrRemoveBlurView`, `copad-macos/Sources/Copad/AlacrittyTerminalViewController.swift::isTransparentBgActive` + `layerBg`, `copad-macos/Sources/Copad/TerminalViewController.swift::applyWindowOpacityToTerminal`, `copad-macos/Sources/Copad/TabBarView.swift::applyWindowOpacity`, `copad-macos/Sources/Copad/StatusBarView.swift::applyWindowOpacity`.

## 57. `[window] opacity` on Linux (GTK4) — resolving the #56 deferral

**Decision.** Wire the existing cross-platform `[window] opacity` to the GTK4 window. The single opaque surface on Linux was `window { background-color: <theme.background hex> }` (the fallback backdrop behind the permanently-transparent VTE + transparent chrome). Switching it to `rgba(theme.background, opacity)` is the whole no-image path: GTK4 derives the toplevel opaque region from rendered content, so a translucent CSS background leaves no opaque region and the compositor shows the desktop through — no `set_opaque_region` / RGBA-visual call needed (that was GTK3). Text glyphs stay opaque because VTE paints them.

**Why scale the image + tint alphas by `opacity` too (macOS parity).** With a background image, the image (`ContentFit::Cover`) covers the window, so an opaque image would hide the desktop even at `opacity < 1.0`. Mirror the macOS fix (decision #56 follow-up, commit `0526fba`): `bg_picture` alpha = `background.opacity * window_opacity`, tint alpha = `tint * window_opacity`. One knob fades the image background too.

**Why the window backdrop goes fully transparent when an image is active.** Avoids double-dim. Layering back-to-front is `desktop → window-bg → image → tint`. If the window-bg stayed at `rgba(bg, w)` behind a now-translucent image, desktop transmission would be `(1 - w)·(1 - io·w)` instead of the intended single-layer fade — a second semi-opaque backdrop stacking behind the image. macOS sidesteps this by setting `NSWindow.isOpaque = false` (clear) when an image is set; the Linux equivalent is `window-bg alpha = 0` whenever an image is active.

**Why `BackgroundLayer` owns the window backdrop CSS.** The backdrop alpha is a function of image-state, which `BackgroundLayer` mutates from two directions: config hot-reload (`apply_config`) *and* runtime socket commands (`background.set`/`next`/`toggle`/clear, which call `set_image`/`clear_image`). An earlier draft refreshed the backdrop only on the config path, so a socket-driven image add/remove at `opacity < 1.0` left a stale backdrop (double-dim, or a fully transparent window after a clear). Making the layer own the `window_css` provider + theme color + a private `refresh_window_backdrop()` — called from `set_image`, `clear_image`, and `apply_config` — means *every* image-state change refreshes it, with no caller able to forget. `window.rs` just creates the provider, registers it on the display, and hands it to `BackgroundLayer::new()`.

**Why `blur` stays a no-op.** No NSVisualEffectView equivalent in GTK; on Wayland, blur behind a translucent surface is the compositor's job. Hyprland `decoration:blur` blurs behind the translucent copad window automatically — so the user gets blur "for free" without the app drawing it. The config key is accepted for cross-platform parity (decision #56) and ignored here.

**Non-finite guard.** `norm_opacity(o) = o.is_finite() ? o.clamp(0,1) : 1.0`. `f64::clamp` propagates NaN, which would poison the CSS string / widget alpha; a malformed TOML float collapses to fully opaque instead. (The Rust core trusts the parse per its `WindowConfig` doc comment; clamping lives at the Linux consumption site, alongside the macOS `clamp01` in `Config.swift`.)
...
## 59. Linux background-image decode moves off the GTK main thread

**Decision.** On Linux, decoding a background image runs on gio's blocking pool (`gio::spawn_blocking`) instead of synchronously on the GTK main thread. Only the ready `MemoryTexture`'s raw pixels (`glib::Bytes` + dimensions) cross back to main, where the texture is built and mounted. A monotonic `load_generation` drops stale decodes, and a single-in-flight + latest-pending coalesce bounds decode work under rapid switches. This mirrors the macOS model (off-main `NSImage` decode + `backgroundLoadToken`), which never had the problem.

**Why.** VTE runs the PTY read/write loop as a GLib IO-watch on the *same* main context that renders — PTY I/O and rendering are not on separate threads on Linux (unlike macOS, where alacritty's `EventLoop` owns a dedicated reader thread). The old path called `gdk::Texture::from_file()`, which reads + decodes synchronously on main. For a large wallpaper the decode blocked the main loop long enough that VTE could not service the PTY, so every background transition stuttered stdin/stdout. Measured A/B on 13756×5756 wallpapers: `background set` socket round-trip 654–902 ms and PTY output stalls up to 1346 ms per switch on the old path, versus 23–56 ms round-trip and ≤25 ms (one frame) PTY gaps on the new path.

**Why synchronous logical state (with a failure rollback).** `current` / `has_image` are updated at request time, not in the decode completion callback — like macOS setting `currentBackgroundPath` before its async decode. This keeps command semantics intact: `background.next` immediately followed by `background.delete_current` operates on the just-picked image (`current_list_image()` reads `current`), not the previous one. Only the *pixels* land asynchronously. The exception is a failed decode: since `current` was already committed to an image that never painted, `on_decode_complete` rolls `current` / `has_image` back to `mounted` (the identity of the last image that actually painted, set only on a successful mount). Without this, `delete_current` could delete a list image that was never displayed — a destructive action on a file that isn't the one on screen.

**Why generation is bumped per transition, but config reloads spare rotation.** `apply_image` and `clear_image` invalidate in-flight decodes via `invalidate_pending()`, or a slow decode could mount after the state moved away and resurrect an unwanted image. Config reloads are subtler: because `current` is synchronous, `showing_list_image()` reflects the *pending* decode's kind, and a config reload must never disturb a rotation (list) pick. So `apply_config` only invalidates a pending **static** decode (`!showing_list_image()` on a missing `[background] image` path, or the static clear via `clear_image` when `image` is unset) — an in-flight list/rotation decode is left to mount. Blanket-invalidating here would drop the final rotated image and leave `current` diverged from the mounted pixels.

**Why coalescing, not just stale-drop.** Stale-drop discards *results*; it does not bound *work*. Without coalescing, `background.next` spam would enqueue a full decode per keypress. Holding at most one in-flight decode plus one queued (latest wins) bounds it — verified: 10 rapid switches produced a single texture mount, the rest superseded before completing.

**Notes.** Decode uses `gdk_pixbuf::Pixbuf::from_file` inside the worker (GDK/pixbuf objects are not `Send`, so the pixbuf is consumed there and never escapes; only `glib::Bytes` crosses back). `apply_embedded_orientation()` is called to match `gdk::Texture::from_file`'s EXIF handling. `gen` is a reserved keyword in Rust 2024 — the field is `load_generation`.

**See:** `copad-linux/src/background.rs::BackgroundLayer::{apply_image, spawn_decode, on_decode_complete, mount_texture, invalidate_pending}`, `decode_image`.
