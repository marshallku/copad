...
| [macos-gpu-renderer-plan.md](./macos-gpu-renderer-plan.md) | Metal render path behind `[renderer] gpu` (renderer-migration Phase 9, gate waived). Slices 1–3 done 2026-06-12/14 — **Metal is the default** (~5.5× cheaper main-thread render); CoreText kept as the `gpu = false` opt-out + fallback | Working on the Metal painter, glyph atlas, or optional slice-2 polish (atlas sharing, present pacing) |
