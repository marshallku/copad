...
- [x] **Background rotation interval timer** — ported 2026-06-12. `[background] rotate_interval` parses into `CopadConfig.rotateInterval` (BackgroundSection CodingKeys `rotate_interval`). `AppDelegate` owns a `DispatchSourceTimer` (`armRotation`/`rotateOnce`): startup applies the static `image` then takes the first rotation tick; manual `background.set`/`next`/`toggle` re-arm to restart the countdown; config hot-reload re-arms with the new interval and `TabViewController.applyConfig` preserves a rotated wallpaper when `image` is unset (`currentBackgroundFromList` guard, parity with Linux's `showing_list_image`). `background.delete_current` deletes the displayed list-picked image (disk + both native/fallback lists via new `copad_ffi_background_remove_from_list` over `copad_core::background::remove_from_list`), then rotates; refuses manual/static images with `no_current`. `next`/`toggle` now tag picks list-sourced (`fromList: true`). Swift package can't compile on the Linux box (no AppKit) — `swift build`/e2e to be run on macOS.
