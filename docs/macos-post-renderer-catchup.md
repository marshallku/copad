...
- [ ] **Background rotation interval timer** — Linux gained native rotation (2026-06-12: `[background] rotate_interval` glib timer + `background.delete_current`, roadmap Phase 2). macOS already shares `pick_random`/`is_active`/`toggle` via `copad_ffi_background_*`; port = a `Timer`/`DispatchSourceTimer` in `AppDelegate` calling `BackgroundRotator.nextRandomImage()` on the interval + a `background.delete_current` handler reusing `copad_core::background::remove_from_list` over FFI.
