...
- [ ] **`copad_term_history(handle, lines)` FFI for true alacritty scrollback** — A8 left `terminal.history` viewport-only on the alacritty backend (returns same content as `terminal.read`). SwiftTerm walks negative-row indices via `Terminal.getLine(row:)`; alacritty equivalent is `Term::grid()` history rows. New FFI `copad_term_history(handle: *mut CopadHandle, lines: usize, out_string: *mut *mut CopadString)` that allocates a `CopadString` with `lines` rows of scrollback (joined by `\n`, blank rows padded with spaces). Plumb through `CopadTermFFI.Handle.history(lines:)` and rewire `AlacrittyTerminalViewController.history(lines:)` to use it; drop the viewport-only fallback at that point. Cargo test on Rust side; manual e2e to confirm `coctl call terminal.history --lines 200` returns scrollback content after running `seq 200`.
