...
- [ ] **OSC 4 color queries** — `Event::ColorRequest(idx, reply_fn)` carries a callback expecting an `Rgb` value materialized from the current palette. We still drop it in `NesttyListener::send_event`'s catch-all arm. Wire-up needs to map `idx` against the active `NesttyTheme.palette` and call `reply_fn(rgb)`, then forward the resulting string via the same `EventLoopSender` path A3 uses.
