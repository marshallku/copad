...
- [x] **NSImage async loading** — both `TerminalViewController.applyBackground` (SwiftTerm path) and `AlacrittyTerminalViewController.applyBackground` (alacritty path) now offload `NSImage(contentsOfFile:)` to `DispatchQueue.global(qos: .userInitiated)` and bounce the visual swap back to main. Per-controller monotonic `backgroundLoadToken` (bumped on every applyBackground / clearBackground) drops stale decodes when a newer load races ahead — same applies if `clearBackground` arrives mid-decode, the late callback finds a token mismatch and no-ops. No fade-in; previous image stays visible until the new one decodes (better than a momentary blank frame).
...
- [x] **Block selection (Cmd+Option+drag)** — `selectionKind(for:)` returns `.block` when `event.modifierFlags.contains(.option)` on a single click (double / triple click stay word / line — keyed off click count). New `NESTTY_SELECTION_BLOCK = 3` constant + `SelectionType::Block` arm in `nestty_term_selection_start`. `paintSelection` checks `sel.is_block` (already on the wire via `selection_range_for_ffi`) and paints a rectangle instead of the row-wrapped span. `selectionString` and copy/paste flows pick up Block automatically because they go through alacritty's `Selection::to_range` + `selection_to_string`.
