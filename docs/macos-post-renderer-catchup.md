...
- [ ] **NSImage async loading** — initial async refactor (`f1352ef`) was reverted in `c9a9d7f` after a "wallpaper never appears" regression that turned out to be unrelated (it was the `transparent_default_bg` default + `TERM=xterm-ghostty` inheritance, not the async itself). Sync path is back in place and works; the async re-attempt is parked until somebody has time to write it with explicit Swift Concurrency (Task / TaskGroup) and proper visual e2e — the `DispatchQueue.global` flow shipped without enough cycles to verify under real Gatekeeper-stall conditions.
