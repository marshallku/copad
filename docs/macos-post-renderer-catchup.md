...
- [x] **DSR / DA reply forwarding** — `nestty-term` `NesttyListener` now matches on `Event::PtyWrite` and forwards the bytes via `EventLoopSender::send(Msg::Input(…))` back to the child PTY. alacritty_terminal already formats DSR (`CSI 6n` → `\e[<row>;<col>R`), DA (`CSI 0c` → `\e[?6c`), color queries (`OSC 4 ; n ; ?`), and other replies; we were dropping all of them on the listener floor. Sender is late-bound (`set_sender` after `EventLoop::channel()`); PtyWrite events before injection drop silently (none can fire that early in practice). Linux unaffected (VTE handles DSR/DA in-widget).
