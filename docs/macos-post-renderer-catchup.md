...
- [ ] **`terminal.feed` / `terminal.exec` for alacritty backend** — `TabViewController.activeTerminal: TerminalViewController?` (SwiftTerm only), so socket commands `terminal.feed` / `terminal.exec` / `terminal.state` / `terminal.read` / `terminal.history` / `terminal.context` silently no-op when the active panel is an `AlacrittyTerminalViewController`. Linux dispatches via `TabManager::find_first_terminal` which is backend-agnostic; macOS needs an `CopadPanel`-protocol-level shim (e.g. `feedText(_:)`, `readScreen()`) implemented by both controllers, then `activeTerminal()` returns the protocol instead of the SwiftTerm class. Discovered while e2e-testing the `panel.report_cwd` shell hook — `coctl call terminal.feed` on the alacritty backend was a no-op, masking the cwd flow.
