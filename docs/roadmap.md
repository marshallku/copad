...
1. **Calendar event imminent** → KB note auto-created with title / start / location interpolated into the frontmatter; the calendar event payload also exposes `event.attendees`, `event.description`, `event.recurring_id`, etc. for users who want a richer template. _(Building blocks shipped — Phase 9.3 KB plugin + Phase 10.1 Calendar plugin; example trigger config at `plugins/calendar/triggers.example.toml`.)_
2. **Slack mention/DM** → archived to KB with full fidelity, optionally summarized via LLM. _(Archive shipped — Phase 11.2; LLM-summarize step blocked on chained-trigger work — Phase 14.)_
3. **Todo with `start` action** → optional Slack message asking for context → wait for reply containing Jira ticket # → **git worktree** created (`~/dev/<workspace>-worktrees/<jira-id>/`) → tmux session opened in the worktree path (attach-or-create so it persists across nestty restarts) → Claude Code spawned **inside the tmux session with a pre-filled prompt** built from the Todo body + Jira summary + linked KB notes. Worktree (not plain branch) so the original repo stays clean and the user can have parallel branches concurrently in different nestty tabs. _(Not yet expressible — needs Phases 15.1, 17, 18, 16, 14.1, 15.2, 14.2 in that order — see "Recommended execution order" below.)_
4. **Jira ticket assigned** → Todo auto-created, frontmatter `linked_jira` populated with the ticket key. _(Not yet expressible — needs Phases 15 + 16. Cross-linking back to related Slack threads is a future enhancement that depends on Phase 11.3's derived ingestion landing first.)_
...
- [x] **WR-3**: docs + ready-to-copy snippet at [`examples/triggers/hyprland-webkit-fix.toml`](../examples/triggers/hyprland-webkit-fix.toml). Two empirical findings from end-to-end testing on Hyprland 0.54.3 (mechanism not characterized — these are observed behaviors, not explained ones): (a) `hyprctl --batch "<cmd1>; <cmd2>"` does NOT cure the freeze, but two SEPARATE `hyprctl dispatch` calls chained with `&&` DO cure; (b) the trigger fires on workspace return regardless of which window has focus on that workspace, so `resizeactive` (active-window target) often resizes an unrelated window. Shipping form uses `resizewindowpixel '<delta>,class:com.marshall.nestty'` — focus-agnostic — wrapped in `sh -c` for the `&&` chain, with `dispatch -- resizewindowpixel` on the second call because `-1 0,...` would otherwise be parsed as a hyprctl flag. The `sh -c` opt-out stays safe only because the snippet has zero `{event.X}` AND zero `{context.X}` interpolations into the shell string — once the user opts into `sh -c`, every interpolation source must be audited (event payload AND context fields), not just the originating event's payload. The example preamble + troubleshooting note + `handle_system_spawn` doc comment all carry this warning. On the dual-monitor DP-1 + HDMI-A-1 setup we tested, cross-monitor workspace switches did NOT trigger the freeze (only same-monitor cycles did); behavior on other multi-monitor configurations not characterized.
...
- [x] Mock `nestty-plugin-echo` (Rust workspace member): `onStartup`, registers `echo.ping` (round-trips params), publishes `system.heartbeat` every `NESTTY_ECHO_HEARTBEAT_SECS` seconds (default 30). Manifest at `plugins/echo/plugin.toml`. E2E verified: socket → registry → service → response (round-trip), `system.heartbeat` events visible via `event.subscribe`, supervisor auto-restarts after `pkill -KILL` of the child.
...
- [x] **Resolved (Phase 9.4 bounded pool, daemon-only v1):** new `nestty_core::thread_pool::ThreadPool` (crossbeam-channel bounded, configurable workers/queue via `NESTTYD_POOL_WORKERS` / `NESTTYD_POOL_QUEUE`; defaults `available_parallelism * 2` clamped to [4, 16] + queue 64). `ActionRegistry::with_pool(pool)` routes blocking handlers through `pool.try_execute`. New `Cancelable` trait so `DispatchJob::cancel` can synthesize an `overloaded` `ResponseError` + publish `<action>.failed` synchronously on the caller thread when the queue is full — no orphaned `Responder`s. Pool worker wraps each job in `catch_unwind` so a panicking handler doesn't sink the worker. `Drop` skips self-join (a job that releases the last `Arc<ThreadPool>` from the worker thread would deadlock otherwise). Wired only from `nesttyd/main.rs`; `nestty-linux`'s in-process registry still uses the legacy spawn fallback (out of scope, follow-up sub-step 9.4.c for the GTK-tick in-process path). E2E `scripts/e2e-daemon-client.sh` step 6: 12 concurrent connections × `__test.slow_blocking` with `NESTTYD_POOL_WORKERS=2 NESTTYD_POOL_QUEUE=2`, asserts 12/12 responses + ≥6 `overloaded`. `daemon.info` now exposes `pool.{workers,capacity,active,queued}`.
- [x] **Step 5b.4 — GUI per-instance socket hardened:** GUI's per-instance socket relocated from `/tmp/nestty-{PID}.sock` (parent `/tmp` is world-writable, only the umask gates `connect(2)`) to `<runtime_dir>/gui-{PID}.sock` (parent already 0700 from the daemon's `prepare_socket_path` machinery) + explicit 0600 on the bound socket. `nestty-core::paths::gui_socket_path(pid)` is the new canonical accessor; `nestty-core::paths::is_legacy_per_instance_socket` recognizes BOTH forms so `daemon_socket_path()` continues to dodge a child shell's inherited `NESTTY_SOCKET` (whether the parent ran an old-form or new-form binary). 4 inject sites in nestty-linux (`window.rs`, `terminal.rs`, `statusbar.rs` ×2, `tabs.rs`) and 1 dispatch site (`socket.rs::start_server`) updated; `nestctl`'s `discover_socket` checks the new hardened pattern first, then falls back to the legacy `/tmp` form, then the daemon well-known path. 2 new unit tests: `gui_socket_path_is_pid_named_under_runtime_dir`, `daemon_socket_ignores_hardened_gui_pattern`. e2e 8/8 still PASS.
- [x] **Step 5b — daemon is the sole plugin host:** `NESTTYD_HOST_PLUGINS` env gate dropped (plugins always run from `nesttyd`). `ServiceSupervisor::new(...)` removed from `nestty-linux/src/window.rs` along with the struct field, import, destroy cleanup, and reserved-method list. Manifest discovery stays in the GUI (panel render, statusbar modules, command lists). **Event bridge:** new `gui_registry::start_event_forwarder` spawns a per-connection forwarder that drains the daemon `EventBus` and writes wire `Event { type, data }` lines on the GUI's socket; the GUI's `gui_client` reader publishes them onto the local bus so the in-GUI `TriggerEngine` continues to see `<action>.completed`/`.failed` events from daemon-hosted plugins (chained-workflow preservation — the plan-round-3 C1 blocker). Forwarder is started AFTER the `gui.register` ack to avoid racing the first event past `await_register_ack`. **GUI dispatch fallback:** new `nestty-linux/src/daemon_forward.rs` (process-lifetime ThreadPool, 4 workers / 16 queue) proxies any unmatched `socket::dispatch` method to the daemon over a fresh `UnixStream` on a worker thread — GTK timer never blocks on a slow plugin reply. Surface vocabulary: `no_daemon` (daemon not reachable), `overloaded` (forward pool saturated). E2E `scripts/e2e-daemon-client.sh` step 5 verifies the event bridge (`__test.slow_blocking.completed` reaches the registered mock GUI within 5s after a separate-connection caller fires the invoke). 7 new unit tests across `gui_registry::tests` (event forwarder writes/stops) and `daemon_forward::tests` (response roundtrip, no_daemon fallback, cancel→overloaded). **Out of scope (separate sub-steps):** trigger engine + ContextService relocation to daemon (5b.2 — needs GUI→daemon event forwarding too); `[[modules]]`/legacy `plugin.<name>.<cmd>` shell execution path (5b.3 — daemon needs to fork/exec instead of plugin RPC); GUI per-instance socket permission hardening (5b.4 — needs socket path relocation under `runtime_dir()`).
- [x] **Step 5a — daemon-attached by default (NESTTY_DAEMON_CLIENT gate removed):** `nestty-linux/src/window.rs` no longer reads the env var; `gui_client::spawn()` always runs. Daemon-absent is graceful: the reconnect_loop polls with 1→30s backoff while the GUI runs entirely through its own in-process supervisor/socket. When a daemon appears, the GUI auto-registers and starts serving daemon→GUI Invokes too. Verified live: launched GUI without daemon (plugins ready, shell spawn, background image render — full UI parity), then started daemon, GUI auto-attached on the next backoff tick. Real-GUI verification also confirmed all of Phase 4b + 9.4 + 9.4.b end-to-end (heartbeat survival, daemon kill→reconnect with new client_id, no UI regression).
- [x] **Resolved (Phase 9.4.b — gui_client.rs Invoke pool):** mirror of 9.4 on the daemon→GUI side. `nestty-linux/src/gui_client.rs` reader replaces `thread::spawn` per Invoke with a process-lifetime `ThreadPool` (POOL_WORKERS=8 / POOL_QUEUE=32). `GuiInvokeJob` (Cancelable): `run` = current `handle_invoke` body; `cancel` = writes back `Response::error(id, "overloaded", …)` directly. `_ping` stays inline on the reader thread (Step 4b heartbeat liveness contract). Pool lives at `spawn()` scope so reconnect doesn't drain a queue of up to 40 in-flight invokes (would block ≤ `ceil(40/8) × 125s`). **Generation token** (`Arc<AtomicU64>`, bumped on every `run()`) gates queued jobs: a worker picking up a job admitted to an older connection writes `overloaded` instead of dispatching, so `tab.new` / `terminal.exec` / `webview.*` etc. cannot side-effect through the GTK pump after the daemon has already failed the corresponding pending invoke. 4 unit tests (run dispatches and writes reply, cancel writes overloaded, cancel with missing id still replies, stale generation skips dispatch).
- [ ] **Known limitation:** the supervisor's `dispatch_invocation` waiter spawn is still per-call. Bounded-pool reuse there is a follow-up; not currently a burst hot spot because plugin invocations are rate-limited by trigger config.
...
- [x] **Phase 9.5 orphan-on-crash hardening — re-shipped clean in step 2c of the daemon-first migration**: `ServiceSupervisor` now routes every plugin spawn through a long-lived `nestty-spawner` thread set up in `ServiceSupervisor::new` (relocated to `nestty-daemon`). Because the spawner thread lives for the supervisor's full lifetime, `prctl(PR_SET_PDEATHSIG, SIGTERM)` re-armed via `Command::pre_exec` (Linux-only) fires when nestty/nesttyd actually dies — not when a transient per-spawn worker exits, which was the trap that forced the Phase 18.x rollback. `shutdown_all` additionally drops the spawner sender and joins the thread before sweeping pids, so any in-flight spawn either completes (child registered, killed by sweep) or is short-circuited by the spawner's post-spawn `shutting_down` re-check (closes the C2 race codex flagged in step 2b review). macOS still relies on cooperative `shutdown_all` (no prctl equivalent); tracked alongside the macOS shell pivot. Tests: `spawner_thread_started_on_new_and_joined_on_shutdown`, `shutdown_all_is_idempotent` plus the pre-existing 37 supervisor tests.
...
- [x] Plugin manifest at `plugins/calendar/plugin.toml`. `onStartup` activation (the polling daemon must be alive whenever nestty is — `onAction:calendar.*` would only spawn on explicit query, too late for "10 minutes before meeting"). Example trigger config at `plugins/calendar/triggers.example.toml` updated in Phase 10.2 to use the new `condition` clause directly (skip-if-declined, skip-1:1-from-common — see 10.2 below).
...
- [x] **Example update**: `plugins/calendar/triggers.example.toml` rewritten to use `condition` directly. Skip-if-declined: `event.my_response_status != "declined"`. Skip-the-weekly-1:1-from-common: `event.recurring_id != "REPLACE_..."`. Both rules now fire only on the events they should — no more multi-rule workaround callout. The 1:1 override has its own skip-when-declined guard.
...
- [ ] **OAuth client credentials must be supplied by the user** (`NESTTY_CALENDAR_CLIENT_ID` / `NESTTY_CALENDAR_CLIENT_SECRET`). Embedding shared OAuth credentials in OSS would let any forked nestty impersonate "nestty" in consent screens. The setup cost (one-time Google Cloud project) is the price of the trust boundary. Documented in `plugins/calendar/plugin.toml`.
...
- [x] **Cross-plugin trigger example** in `plugins/slack/triggers.example.toml`: `:memo:` reaction → `slack.get_message` → `todo.create` chain via Phase 14.2 await for correlation (without the synthesized `<trigger_name>.awaited` event, every successful `get_message` would trigger every reaction-driven todo).
...
Same shape as Slack plugin — REST + auth + events + actions. Two slices.

**Phase 16.1 — auth + manifest + skeleton** (slice 1) — **shipped**:

- [x] **`nestty-plugin-jira` Rust workspace member** (Linux + macOS via `compile_error!` Unix-only gate, same posture as calendar/slack/discord — keyring's mock fallback would silently lose tokens otherwise).
- [x] **Auth**: API token via env (`NESTTY_JIRA_BASE_URL` / `_EMAIL` / `_API_TOKEN`) or keyring. Atlassian Cloud only — `validate_base_url` restricts host to `<workspace>.atlassian.net`, rejects userinfo/port/path/non-Atlassian hosts so a misconfigured env can't exfiltrate the API token to an attacker. Stored TokenSet `{email, api_token, base_url, account_id, display_name}` carries base_url so unsetting env post-auth still works. Same `*.atlassian.net` validation re-applied at store-load time so a tampered keyring entry can't bypass the env-time gate. Whitespace-only env values trimmed → treated as empty (won't shadow the store). Cross-checks env email vs `/myself`'s emailAddress (warn-only — privacy-controlled workspaces return empty email).
- [x] **`jira.auth_status` action**: `{configured, authenticated, credentials_source ("env"|"store"|"none"), fatal_error, store_kind, workspace, base_url, account_id, display_name}`. Identity (account_id, display_name) only surfaced when source=="store" — env-supplied creds haven't been validated by `/myself` so reporting stored identity for them would be misleading. fatal_error short-circuits to `configured=false` so callers can distinguish bad-env from no-creds.

**Phase 16.2 — polling loop + events + actions + triggers** (slice 2) — **shipped**:

- [x] **Polling daemon** in `poller.rs` (~470 LOC). JQL: `(assignee = currentUser() OR watcher = currentUser()) AND updated > -<NESTTY_JIRA_LOOKBACK_HOURS>h[ AND project in (...)] ORDER BY updated DESC`. Uses Atlassian's new `POST /rest/api/3/search/jql` endpoint (cursor-based via `nextPageToken`) — the legacy `GET /rest/api/3/search` was deprecated in 2024 with sunset by 2025. Paginates via `nextPageToken` capped at 20 pages × 100 issues. Search uses `fields=["*all"]` so `event_json` carries every custom field (trade-off: ~5-10× larger response than a curated field list — acceptable for v1's typical few-dozen-tickets workspace). Configurable: `NESTTY_JIRA_POLL_SECS` (default 300, range 60..=3600), `NESTTY_JIRA_LOOKBACK_HOURS` (default 24, range 1..=720), `NESTTY_JIRA_PROJECTS` (optional comma-separated allowlist with strict `[A-Z][A-Z0-9_]*` validation), `NESTTY_JIRA_FETCH_COMMENTS` (default true; opt-out skips per-ticket `/comment` calls when comment_count delta detected). First tick runs immediately (matches calendar — without this an event whose firing window overlaps startup is permanently missed). `my_account_id` resolved atomically with credential snapshot via `ResolvedCreds.account_id_hint` — stored TokenSet's account_id when source=="store" (auth subcommand recorded it), one-off `/myself` for env-source. Cached for the rest of the poller's lifetime.
- [x] **4 event kinds**: `jira.ticket_assigned` (first sight assigned to me, OR assignee transitions to me), `jira.status_changed` (after baseline; carries `from`/`to`), `jira.comment_added` (per inline comment whose `created` timestamp is strictly newer than the per-ticket watermark; carries full `comment` sub-object with body in plain text + ADF), `jira.mention` (alongside comment_added when ADF tree contains a `mention` node with `attrs.id == my_account_id`; self-mentions filtered to prevent feedback loops with auto-reply triggers). Common envelope: `{key, summary, status, assignee_account_id, assignee_display, reporter_account_id, project_key, url, updated, event_json}` — `event_json` is the verbatim Jira response (matches the slack/discord raw-event convention).
- [x] **Comment delivery uses timestamp watermark, not index**: Jira's comment indices SHIFT when an old comment is deleted. A `startAt=N` fetch after a deletion would skip the new comment that shifted into position N. We instead read the inline `fields.comment.comments[]` array (returned by `*all` searches) and emit for any comment whose `created` is strictly newer than the per-ticket watermark — robust against arbitrary add/delete patterns.
- [x] **First-sight rule**: snapshot=None → emit `ticket_assigned` if currently mine, but NOT `status_changed` (no baseline `from` to report). Documented as intentional in the poller doc-comment.
- [x] **Dedupe** via `HashSet<(key, kind, discriminator)>` keyed on the discriminator that makes each kind exactly-once: `ticket_assigned` uses ticket's `updated_iso` (one fire per Jira-side update), `status_changed` uses `<from>:<to>`, `comment_added`/`mention` use comment id. Cap 4096; flush past cap (matches calendar's posture — accepts re-firing a few boundary events over unbounded growth).
- [x] **5 actions**: `jira.list_my_tickets {status?, project?, updated_since?}` (paginated up to 10 pages × 100 via cursor `nextPageToken`; returns `{tickets, truncated}` where `truncated=true` means we hit MAX_PAGES and the user has more results than the action returned; new `/search/jql` API drops `total` so the response no longer reports a count; project key validated via shared charset, JQL string args double-quote-escaped), `jira.get_ticket {key}` (returns the verbatim Jira issue response with `?fields=*all&expand=changelog,renderedFields` — full custom fields, changelog history, rendered HTML; the trigger-envelope shape is reachable via `list_my_tickets` for callers who want it; **caveat**: Jira caps inline comments at 50 per `/issue/{key}` response with no `?maxComments` knob, so tickets with >50 comments would need a separate paginated comment fetch — `jira.get_comments` action can land in a follow-up if a real workflow needs it), `jira.create_ticket {project, summary, description?, assignee?, parent?, issue_type?}` (description/comment auto-wrapped in ADF — Jira Cloud rejected wiki markup in 2020-06; assignee is accountId per GDPR-driven email-removal in 2018; parent key validated; `issue_type` defaults to `"Task"` because company-managed projects 400 without it — caller passes `"Bug"`/`"Story"`/etc. to override, or any custom type configured in their project), `jira.transition {key, status}` (looks up matching transition by destination status name case-insensitive — surfaces `transition_not_available` with available targets when no match), `jira.add_comment {key, body}` (returns comment_id).
- [x] **Error code conventions** (mirror Slack's bare-snake-case promotion in `map_jira_error`): `unauthorized` (401), `forbidden` (403), `not_found` (404), `rate_limited` (429, with `(Retry-After: <s>)` suffix when header present), `transition_not_available` (custom for transition lookups), `io_error` (other 4xx/5xx, transport failures). Plus action-level `invalid_params` for bad client input. Issue keys validated as `[A-Z][A-Z0-9_]*-[0-9]+` before splicing into URLs (trust-boundary defense against path-injection attempts via crafted keys like `PROJ/../foo`).
- [x] **Example triggers** at `plugins/jira/triggers.example.toml`: `jira.ticket_assigned → todo.create` (with `linked_jira` populated → composes with vision-flow-3 worktree+claude chain), `jira.mention → kb.ensure` (raw archive at `.raw/jira/<key>-comment-<id>.md` — uses ensure not append-with-ensure for true dedup of Jira-side delivery retries). Plus commented sketches for status_changed → slack.post_message and per-project trigger narrowing via condition.
- [x] **89 unit tests across 5 modules**. config (16): env parsing for 5 env vars, base_url Atlassian-only enforcement, workspace label charset, project key validation, parse_bounded_int range checks, parse_projects dedup. store (5): plaintext roundtrip + 0600 perms, concurrent saves, broken store, malformed JSON, unreadable-store distinct from missing. jira (23): base64 against RFC 4648 vectors, basic auth, URL join no-double-slash, status code classification, /myself parse, ADF wrap (single/multi/empty), search response pagination + missing-fields, JQL builder with/without projects, issue key + project key validation. event (15): from_jira_json canonical/missing-assignee/missing-comment-field, to_payload_json envelope, snapshot, ADF→text walk + mention detection (nested lists), parse_comment, jira-timestamp parser handles `+0000` AND `+00:00`. poller (9): first-sight assigned-to-me/other/no-status-changed, assignee transition, status change, dedup blocks repeat, dedup cap flush, skips on fatal_error, skips on no-creds. main (21): auth_status fatal_error short-circuit + env-source-hides-identity + store-source-reports-identity + no-creds, current_credentials env vs store + rejects-incomplete + rejects-non-atlassian-base_url, handle_action routes/short-circuits, required_string/optional_string validators, jql_escape backslash + quote, map_jira_error promotes known prefixes.

**Cross-plugin trigger flow now reachable:** Jira ticket assigned → Todo created with `linked_jira` populated → user clicks Start in the Todo panel → vision-flow-3 chain (`todo.start_requested → git.worktree_add { sanitize_jira: true } → git.worktree_add.completed → claude.start`) stages a worktree + tmux + claude session with the layered prompt.

**Known limitations of 16.2, tracked for follow-up:**

- [ ] **Inline comment cap (50 per `/search` ticket)**: Jira returns at most 50 inline comments per ticket in search responses. If a ticket gains >50 comments newer than the watermark in a single poll interval, the events for the comments that fell off the inline window are silently lost. Detection signal: when the oldest inline comment is newer than the watermark. Closing the gap requires falling back to the paginated `/issue/{key}/comment?startAt=N&orderBy=-created` endpoint — deferred until a real workflow hits it.
- [ ] **GTK-blocking action calls** (Phase 9 known limitation): `jira.list_my_tickets` and friends run synchronously on the supervisor thread. With a slow Jira API response (>200ms), the GTK timer thread stalls during the wait. Inherited from the supervisor design; lands when blocking handlers gain async support.
- [ ] **Multi-account v1**: `NESTTY_JIRA_WORKSPACE` namespaces the keyring entry but the plugin only reads from one account at a time. Multi-account would mean spawning N plugin instances with distinct workspace labels, which the supervisor doesn't yet model.
- [ ] **`nestctl jira` subcommand** (Phase 19.1 work): not part of 16.x — lands separately to mirror how `nestctl git` followed Phase 17.1.
- [ ] **Self-hosted Jira Server / Data Center**: intentionally rejected at validation time. Different auth (PAT or session cookies, not Basic + API token), different REST surface; add a `Server` adapter when there's concrete demand.
- [ ] **OAuth 3LO**: deferred until env+keyring proves insufficient (same posture as Slack/Discord).
...
- [x] **Plugin manifest** at `plugins/bookmark/plugin.toml`. `onAction:bookmark.*` activation (no panel/watcher in BM-1; BM-2 may flip to `onStartup` for the fetch worker).
...
- [x] **Plugin manifest** at `plugins/discord/plugin.toml`. `onStartup` activation (Gateway WebSocket lives whenever nestty runs). Slice 1 shipped `provides = ["discord.auth_status"]`; slice 2 added `discord.send_message`. Required scopes / OAuth flow / MESSAGE CONTENT intent setup documented inline in the manifest.
...
- [x] **Cross-plugin trigger examples** in `plugins/discord/triggers.example.toml`: raw archive (kb.ensure dedup), mention → personal-tasks file with `condition` to skip @everyone, DM → llm.complete → reply (Phase 14.2 await chain sketch), single-channel subscription via `payload_match { channel_id = "..." }`.
...
- [x] `api.rs::classify_response_error` shared between every REST helper so the failure surface (rate*limited / discord*<numeric> / io_error) stays uniform.
- [x] `plugins/discord/triggers.example.toml` adds the canonical "react with 📝 → Todo" recipe using Phase 14.2 `await` to correlate the `get_message.completed` reply with the originating reaction trigger (without correlation, every `get_message` would trigger every reaction-driven todo).
