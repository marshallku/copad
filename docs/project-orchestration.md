...
> Status: **largely superseded — historical design.** Phases 22.2–22.7 implemented the surfaces sketched here (decision [#48](./decisions.md#48-copad-native-port-of-project-orchestration-spine)), but dogfooding + the `csd` direction showed the goal-tick/mission spine was over-built for copad's single-track coding use. [Decision #51](./decisions.md#51-orchestration--lean-goal-queue-plugin-over-csd-goal-tick-state-machine--mission-removed) replaces it with a lean **goal-queue plugin** over `csd` and **removes** `copad-core::goal`/`::mission` (`agent`/`approval`/`pipeline` flagged) in Phase 24.1/24.7. Read §§2–9 below as the now-retired spine design; the live plan is [roadmap.md § Phase 24](./roadmap.md#phase-24-csd-integration--autonomous-loop-the-body-for-the-224227-spine) + decisions #49/#50/#51. The `csd` dispatch note in §5 still applies.
