...
/// UI-side model. Field layout mirrors `nestty_core::theme::Theme` exactly.
/// Palette data lives in Rust; this struct is populated by decoding the JSON
/// returned by `nestty_ffi_theme_get`.
struct NesttyTheme {
    let name: String
    ...
    /// `nestty_ffi_theme_get` returns NULL and the caller (currently
    /// `AppDelegate.applyConfig`) falls back to `.default`.
    static func byName(_ name: String) -> NesttyTheme? {
        guard let cstr = name.withCString({ nestty_ffi_theme_get($0) }) else { return nil }
        defer { nestty_ffi_free_string(cstr) }
        return decode(jsonCString: cstr)
    }

    /// Matches `nestty_core::theme::Theme::default()` ("catppuccin-mocha").
    /// Force-unwrap is intentional: a missing default means the FFI link
    /// or `nestty_core::theme` is broken and the app cannot render — fail
    /// loud rather than silently substitute black-on-black.
    static var `default`: NesttyTheme {
        guard let theme = byName("catppuccin-mocha") else {
            preconditionFailure("nestty-core failed to return catppuccin-mocha theme — FFI broken")
        }
        return theme
    }
    ...
    private static func decode(jsonCString: UnsafePointer<CChar>) -> NesttyTheme? {
