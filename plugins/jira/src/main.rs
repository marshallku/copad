...
const CONFIG_REMEDIATION: &str = "set COPAD_JIRA_BASE_URL, COPAD_JIRA_EMAIL and \
     COPAD_JIRA_API_TOKEN, or run `copad-plugin-jira auth`";
...
fn emit_config_error(tx: &Sender<String>, message: &str) {
    let frame = json!({
        ...
        "params": {
            ...
            "payload": { "service": "jira", "message": message, "remediation": CONFIG_REMEDIATION },
        }
    });
    let _ = tx.send(frame.to_string());
}
