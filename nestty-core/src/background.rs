...
//! XDG vs macOS's `~/Library/Caches/nestty`). Callers pass the resolved
...
/// platform try a native path first (e.g. macOS `~/Library/Caches/nestty/`)
...
mod tests {
    ...
    fn tmpfile(name: &str, body: &str) -> PathBuf {
        let p = std::env::temp_dir().join(format!("nestty-bg-test-{name}-{}", std::process::id()));
    ...
    fn toggle_flips_and_persists() {
        let m = std::env::temp_dir().join(format!("nestty-bg-mode-{}", std::process::id()));
