//! Daemon-client thread: connects to `copadd`, advertises GUI
...
use copad_core::event_bus::{Event as BusEvent, EventBus, RecvOutcome, next_bridge_id};
use copad_core::protocol::{Event as WireEvent, Invoke, Request, Response};
use copad_core::thread_pool::Cancelable;
use serde_json::Value;

use crate::socket::SocketCommand;

const PROTOCOL_VERSION: u32 = copad_core::protocol::PROTOCOL_VERSION;
...
/// (see `copad-daemon::gui_registry::GUI_ENV_ALLOWED_KEYS`), so this
...
pub fn spawn(
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let pool = copad_core::thread_pool::ThreadPool::new(POOL_WORKERS, POOL_QUEUE);
            let generation = Arc::new(AtomicU64::new(0));
            reconnect_loop(dispatch_tx, pool, generation, event_bus, host_triggers_tx);
        })
        .expect("spawn copad-gui-client");
}

fn reconnect_loop(
    dispatch_tx: Sender<SocketCommand>,
    pool: std::sync::Arc<copad_core::thread_pool::ThreadPool>,
    ...
    loop {
        // daemon_socket_path filters inherited per-instance COPAD_SOCKET
        // and refuses untrusted runtime dirs.
        let Some(socket_path) = copad_core::paths::daemon_socket_path() else {
            log::debug!(
...
fn run(
    ...
    let stream = UnixStream::connect(socket_path)
        .map_err(|e| format!("connect to copadd at {socket_path}: {e}"))?;
    ...
    loop {
        let line = match copad_daemon::socket::read_line_capped(&mut reader, &mut frame_buf) {
            Ok(Some(l)) => l,
            Ok(None) => return Ok(()),
        ...
        let value: Value = match serde_json::from_str(&line) {
            Ok(v) => v,
            Err(e) => {
                log::debug!("[copad] gui_client malformed line: {e}");
                continue;
            }
        };
        ...
        if let Some(method) = value.get("invoke").and_then(|v| v.as_str()) {
            ...
            if method == "_ping" {
                if let Err(e) = handle_ping(value, &writer_tx) {
                    log::warn!("[copad] gui_client ping reply: {e}");
                }
                continue;
            }
            ...
            log::debug!("[copad] gui_client response: {value}");
            ...
            match serde_json::from_value::<WireEvent>(value) {
                ...
                Err(e) => log::debug!("[copad] gui_client malformed Event: {e}"),
            }
        } else {
            log::debug!("[copad] gui_client ignoring: {line:.200}");
        }
    }
}
...
fn start_gui_event_forwarder(
    ...
    for kind in GUI_TO_DAEMON_FORWARD_KINDS {
        ...
        let thread_name = format!("copad-gui-forwarder-{}", kind.replace('.', "-"));
        if let Err(e) = thread::Builder::new()
            .name(thread_name)
            .spawn(move || gui_forwarder_loop(kind, rx, writer_tx, stop))
        {
            log::warn!("[copad] gui_client: failed to spawn forwarder thread: {e}");
        }
    }
}
        ...
        match rx.recv_timeout(FORWARDER_POLL) {
            RecvOutcome::Event(ev) => {
                ...
                let line = match serde_json::to_string(&req) {
                    Ok(s) => s,
                    Err(e) => {
                        log::warn!("[copad] forwarder({kind}) serialize: {e}");
                        continue;
                    }
                };
...
fn await_register_ack(
    ...
    let line = match copad_daemon::socket::read_line_capped(reader, &mut buf) {
        Ok(Some(l)) => l,
    ...
    if !resp.ok {
        let err = resp.error.unwrap_or(copad_core::protocol::ResponseError {
            code: "unknown".into(),
            message: String::new(),
        });
    ...
    let host_triggers = result
        ...
        .unwrap_or(false);
    log::info!("[copad] gui_client registered with copadd: {result}");
...
impl GuiInvokeJob {
    fn write_overloaded(value: &Value, writer_tx: &SyncSender<String>) {
        ...
        match serde_json::to_string(&resp) {
            ...
            Err(e) => log::warn!("[copad] gui_client cancel serialize: {e}"),
        }
    }
}
...
impl Cancelable for GuiInvokeJob {
    fn run(self: Box<Self>) {
        ...
        if let Err(e) = handle_invoke(this.value, &this.dispatch_tx, &this.writer_tx) {
            log::warn!("[copad] gui_client invoke worker: {e}");
        }
    }
