...
/// Fire a `_module.run` RPC to copadd and yield the module's stdout
...
fn invoke_module(plugin: &str, module: &str) -> Option<String> {
    ...
    // per-GUI COPAD_SOCKET override and refuses untrusted runtime dirs
    // — same guard `gui_client` uses. None on no-daemon → blank label.
    let socket_path = copad_core::paths::daemon_socket_path()?;
...
impl StatusBar {
    pub fn new(config: &CopadConfig, plugins: &[LoadedPlugin]) -> Self {
        ...
        eprintln!(
            "[copad] statusbar modules: left={}, center={}, right={}",
            left_entries.len(),
            center_entries.len(),
            right_entries.len()
        );
        ...
        bar.add_css_class("copad-statusbar");
    ...
    pub fn reload(&self, config: &CopadConfig, plugins: &[LoadedPlugin]) {
        let theme = Theme::by_name(&config.theme.name).unwrap_or_default();
...
fn schedule_modules(modules: &Rc<RefCell<Vec<ModuleHandle>>>) {
    ...
    eprintln!(
        "[copad] statusbar: scheduling {} modules",
        modules_ref.len()
    );
    for module in modules_ref.iter() {
        eprintln!(
            "[copad] statusbar: module {}.{} interval={}s",
