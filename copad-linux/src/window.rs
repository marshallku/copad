...
use copad_core::event_bus::{Event as BusEvent, EventBus as CoreEventBus, EventReceiver};
use copad_core::trigger::{Trigger, TriggerEngine, TriggerSink, covering_patterns};

use copad_daemon::trigger_sink::LiveTriggerSink;
...
impl CopadWindow {
    pub fn new(app: &Application, config: &CopadConfig) -> Self {
        ...
        // handshaking while copad does the rest of its init in
        ...
        // second copad process happens to wake them. See commit
        // 78ebdb1 for the diagnostic instrumentation, and the
        // `prewarm_webview` field on `CopadWindow` for the lifetime
        // contract.
        //
        ...
        // window's lifetime. Temp file is per-pid so concurrent copad
        // instances don't collide; cleaned up on window destroy.
        let prewarm_path =
            std::env::temp_dir().join(format!("copad-prewarm-{}.html", std::process::id()));
        ...
        if let Err(e) = std::fs::write(&prewarm_path, b"<!doctype html><title>p</title>") {
            eprintln!(
                "[copad] prewarm: failed to write {}: {e} — cold-boot \
            ...
            actions.register_silent("event.history", move |params| {
                ...
                let arr: Vec<serde_json::Value> = events
                    ...
                    .map(|e| {
                        ...
                        // `copad-core/src/protocol.rs`) with an added
        ...
        {
            let notifier = copad_core::notifier::platform_notifier();
        ...
        eprintln!(
            "[copad] trigger engine: {} configured ({:?}) | {} bus pattern(s) subscribed",
            triggers.count(),
            triggers.names(),
        ...
        for p in &plugins {
            eprintln!(
                "[copad] plugin loaded: {} v{}",
                p.manifest.plugin.name, p.manifest.plugin.version
            );
        }

        // Plugin lifecycle belongs to copadd; the manifests discovered
        ...
        // Always attempt to attach to copadd. Daemon-absent is fine:
        ...
        window.connect_realize(move |w| {
            let Some(surface) = w.surface() else {
                eprintln!("[copad] window.restored: realize fired with no surface — disabled");
                return;
            };
            let Ok(toplevel) = surface.downcast::<gdk::Toplevel>() else {
                eprintln!(
                    "[copad] window.restored: surface is not a gdk::Toplevel — disabled (compositor/backend mismatch?)"
                );
                return;
            };
            // Seed `last_suspended` with the toplevel's CURRENT state so
            // a window that's already SUSPENDED at attach time (e.g.,
            // copad launched on a non-current Hyprland workspace) still
            ...
            toplevel.connect_state_notify(move |tl| {
                ...
                last_fire.set(now);
                eprintln!("[copad] window.restored: SUSPENDED bit cleared, publishing event");
...
fn watch_config(
    ...
    monitor.connect_changed(move |_, _, _, event| {
        ...
        let config = match CopadConfig::load() {
            Ok(c) => c,
            Err(e) => {
                eprintln!("[copad] config reload error: {}", e);
                return;
            }
        };
        ...
        *cached.lock().unwrap() = config.triggers.clone();
        ...
        if ht_active.load(Ordering::SeqCst) {
            eprintln!("[copad] triggers reload deferred to daemon (host_triggers_active)");
            return;
        }
        ...
        eprintln!(
            "[copad] triggers reloaded: {} active ({:?}) | {} bus pattern(s) subscribed",
            trg.count(),
            trg.names(),
...
impl TriggerSubscriptions {
    ...
    pub fn drain_into<F: FnMut(BusEvent)>(&self, mut f: F) {
        ...
        // Trust-bound priority: only `copad.action`-sourced
        ...
        events.sort_by_key(|e| {
            let is_completion_fan_out = e.source
                == copad_core::action_registry::COMPLETION_EVENT_SOURCE
...
mod tests {
    ...
    use copad_core::trigger::{SecurityBlock, TriggerEngine, TriggerSink, WhenSpec};
