...
/// Mirrors `copad-core::action_registry::ActionRegistry` for macOS. Two seams
/// it owns:
///
...
/// in `copad-core/src/action_registry.rs`): legacy match-arm dispatch in
...
/// copad-internal actions that landed here (`system.*`, `context.*` once
/// it ships).
///
...
final class ActionRegistry {
    ...
    /// across calls so consumers (e.g. `coctl call system.list_actions`)
    ...
    /// `publish_completion` (`copad-core/src/action_registry.rs:281`):
    ...
    /// Mirror of `copad_core::action_registry::COMPLETION_EVENT_SOURCE`.
    ...
    nonisolated static let completionEventSource = "copad.action"
}
