...
extension Notification.Name {
    static let terminalTitleChanged = Notification.Name("CopadTerminalTitleChanged")
}

// MARK: - CopadTerminalView
...
class CopadTerminalView: LocalProcessTerminalView {
    ...
    private var delegateProxy: CopadTerminalDelegate?
    ...
    func installDelegateProxy(policy: OSC52Policy) {
        let proxy = CopadTerminalDelegate(host: self, policy: policy)
        delegateProxy = proxy
...
/// override` (not `open`).
@MainActor
private final class CopadTerminalDelegate: NSObject, @preconcurrency TerminalViewDelegate {
    ...
    func clipboardCopy(source _: TerminalView, content: Data) {
        switch policy {
        case .deny:
            let msg = "[copad] OSC 52 clipboard write blocked (\(content.count) bytes). " +
...
class TerminalViewController: NSViewController, CopadPanel, Zoomable, TerminalCapable {
    ...
    private(set) var terminalView: CopadTerminalView?

    /// Focus target for `panel.focusTarget` — SwiftTerm's `TerminalView`
    /// (wrapped here as `CopadTerminalView`) is the actual keyboard
    ...
    /// in-shell `copad-cwd` hook reports a new cwd. Same setter the
    ...
    override func loadView() {
        ...
        let tv = CopadTerminalView(frame: frame)
    ...
    /// swap-back on the monotonic `backgroundLoadToken` so a stale
    /// decode can't clobber a newer applyBackground / clearBackground.
    ...
    private func applyCaretColors(tv: LocalProcessTerminalView, theme: CopadTheme) {
    ...
    private func startShell() {
        ...
        let socketPath = "/tmp/copad-\(pid).sock"
        ...
        env.append("COPAD_SOCKET=\(socketPath)")
        env.append("COPAD_PANEL_ID=\(panelID)")
