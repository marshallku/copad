...
fn main() {
    ...
    // COPAD_SOCKET + discover_socket as a side effect).
    ...
    let socket_path = cli.socket.clone().unwrap_or_else(|| {
        std::env::var("COPAD_SOCKET")
            .ok()
            .filter(|p| std::os::unix::net::UnixStream::connect(p).is_ok())
            .unwrap_or_else(|| discover_socket().unwrap_or_else(|| "/tmp/copad.sock".to_string()))
    });

    if std::env::var("COPAD_DEBUG_SOCKET").is_ok() {
        eprintln!("[coctl] using socket: {socket_path}");
    }
...
/// 2. **Legacy GUI sockets** under `/tmp/copad-{PID}.sock` — kept so
///    a still-running pre-5b.4 copad stays addressable.
...
fn discover_socket() -> Option<String> {
    let runtime_dir = copad_core::paths::runtime_dir();
    let trusted = copad_core::paths::is_trusted_dir(&runtime_dir);

    if trusted && let Some(hit) = best_connectable_with_prefix(&runtime_dir, "gui-", ".sock") {
        return Some(hit);
    }
    if let Some(hit) = best_connectable_with_prefix(std::path::Path::new("/tmp"), "copad-", ".sock")
    {
        return Some(hit);
    }
    if trusted {
...
fn dispatch_publish(kind: &str, payload: Option<&str>, json: bool, quiet: bool) -> i32 {
    ...
    // even when copadd is down, otherwise the hook author never sees
    ...
    let Some(socket_path) = copad_core::paths::daemon_socket_path() else {
        if !quiet {
            eprintln!(
                "Error [no_daemon]: daemon socket path is untrusted or runtime dir missing; is copadd running?"
            );
        }
