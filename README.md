...
- **One-command server restart** — `comux server restart` (also `start`/`stop`/`status`) stops the server with a final save and brings a fresh one up with your workspace restored — no manual `kill-server`-then-reattach dance when you upgrade the binary or need to clear drift
